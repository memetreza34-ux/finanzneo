...
// Whisper-Words flach + auf den Short-Bereich (≤34,5s) begrenzt
const WORDS = (capData as any).segments
  .flatMap((s: any) => s.words.map((w: any) => ({ word: w.word.trim(), start: w.start, end: w.end })))
  .filter((w: any) => w.start < 34.5);
  ...
  return (
    <AbsoluteFill>
      ...
      <Stage a={T.negation} b={T.end} style={{ marginTop: -120 }}>
        {['Kein Trick.', 'Kein Glück.', 'Keine Zockerei.'].map((t, i) => (
          <div key={i} style={{ fontFamily: FONT.title, fontSize: 78, color: a(C.negative, 0.9),
            opacity: prog(f, sec(T.negation + i * 0.85), sec(T.negation + i * 0.85 + 0.4)),
            transform: `translateY(${lerpF(f, 16, 0, sec(T.negation + i * 0.85), sec(T.negation + i * 0.85 + 0.4))}px)` }}>
            {t}
          </div>
        ))}
