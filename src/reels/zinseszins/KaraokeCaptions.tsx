...
export const ZinseszinsKaraokeCaptions:React.FC=()=>{
  const frame=useCurrentFrame();
  if(ZINSESZINS_WORD_TIMINGS.length===0)return null;
  if(frame<ZINSESZINS_WORD_TIMINGS[0].frames[0])return null;

  let sentenceIndex=0;
  for(let index=1;index<ZINSESZINS_WORD_TIMINGS.length;index+=1){
    if(frame>=ZINSESZINS_WORD_TIMINGS[index].frames[0])sentenceIndex=index;
    else break;
  }

  const sentence=ZINSESZINS_WORD_TIMINGS[sentenceIndex];
