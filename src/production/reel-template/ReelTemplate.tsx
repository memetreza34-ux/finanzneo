...
const SourceNote: React.FC<{children?: string}> = ({children}) => {
  ...
  return (
    <div style={{
      ...
      lineHeight: 1.22, textAlign: 'center', textShadow: '0 2px 1px rgba(0,0,0,0.9)',
    }}>
      {children}
    </div>
  );
...
export const ReelTemplate: React.FC<{config: ReelConfig}> = ({config}) => {
  ...
  return (
    <AbsoluteFill style={{background:C.bg}}>
      ...
      <Series>
        {config.beats.map((beat)=>(
          <Series.Sequence key={beat.id} durationInFrames={beat.durationInFrames}>
            <SceneContinuityFrame durationInFrames={beat.durationInFrames}><ReelBeatView beat={beat}/></SceneContinuityFrame>
          </Series.Sequence>
        ))}
      </Series>
      ...
      {config.captions && config.captions.length > 0 && <Captions words={config.captions} size={64} bottom={285} left={72} right={140} />}
