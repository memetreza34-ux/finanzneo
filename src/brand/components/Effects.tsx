...
}> = ({ to, start, end, size = 160, color = C.accent, suffix = ' €', style }) => {
...
export const Confetti: React.FC<{ width: number; height: number; at: number; n?: number }> = ({
  ...
  const cols = [C.accent, C.gold, C.blue, C.purple, C.white];
  return (
    <AbsoluteFill style={{ pointerEvents: 'none', overflow: 'hidden' }}>
      {Array.from({ length: n }, (_, i) => {
...
export const AuroraBG: React.FC<{ colors?: string[] }> = ({ colors = [C.accent, C.blue, C.accentDk] }) => {
  const f = useCurrentFrame();
...
export const PulseGrid: React.FC<{ color?: string }> = ({ color = C.accent }) => {
  const f = useCurrentFrame();
