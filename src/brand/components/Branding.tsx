  ...
  return (
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center' }}>
      ...
      <svg width={360} height={360} style={{ position: 'absolute' }}>
        <circle cx={180} cy={180} r={150} fill="none" stroke={C.accent} strokeWidth={8}
          strokeDasharray={`${2 * Math.PI * 150}`} strokeDashoffset={`${2 * Math.PI * 150 * (1 - ring)}`}
          transform="rotate(-90 180 180)" style={{ filter: `drop-shadow(0 0 16px ${a(C.accent, 0.7)})` }} />
      </svg>
      <div style={{ display: 'flex', fontFamily: FONT.title, fontSize: 120, lineHeight: 1 }}>
        ...
        <span style={{ color: C.accent, opacity: neo, transform: `translateX(${lerpF(f, 40, 0, at + 18, at + 30)}px)`,
          textShadow: `0 0 40px ${a(C.accent, 0.6)}` }}>Neo</span>
      </div>
    </AbsoluteFill>
  );
};
  ...
  return (
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ marginTop: 50, padding: '24px 48px', borderRadius: 24, opacity: card,
        transform: `scale(${card})`, background: a(C.accent, 0.14), border: `2.5px solid ${C.accent}`,
        fontFamily: FONT.body, fontWeight: 800, fontSize: 46, color: C.accent }}>
        👉 {nextHint}
      </div>
    </AbsoluteFill>
  );
