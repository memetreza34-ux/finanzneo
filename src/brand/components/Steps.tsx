...
export const NumberedSteps: React.FC<{
  ...
  color?: string; gap?: number;
}> = ({ steps, color = C.accent, gap = 38 }) => {
      ...
      {steps.map((s, i) => {
        ...
        return (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 32,
            ...
            <div style={{ fontFamily: FONT.body, fontWeight: 700, fontSize: 56, color: C.white }}>{s.label}</div>
          </div>
        );
      })}
    </div>
        ...
        return (
          <div key={i} style={{ position: 'relative', opacity: p,
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 26, padding: '40px 42px',
              borderRadius: 28, background: on ? a(C.accent, 0.12) : 'rgba(255,255,255,0.05)',
              border: `2.5px solid ${on ? C.accent : 'rgba(255,255,255,0.2)'}` }}>
              <div style={{ fontSize: 56 }}>{on ? '✅' : '❓'}</div>
              <div style={{ fontFamily: FONT.body, fontWeight: 700, fontSize: 52,
                color: on ? C.accentLt : C.white, lineHeight: 1.15 }}>{c.text}</div>
            </div>
            {solved > 0 && (
              <div style={{ position: 'absolute', top: -20, right: -8,
                transform: `scale(${solved}) rotate(-8deg)`, background: C.accent, color: C.bg,
                fontFamily: FONT.title, fontSize: 30, padding: '8px 20px', borderRadius: 10,
                boxShadow: `0 0 22px ${a(C.accent, 0.7)}`, letterSpacing: 1 }}>GELÖST</div>
            )}
          </div>
        );
      })}
...
export const Timeline: React.FC<{
  ...
  color?: string;
}> = ({ width, y, left, right, points, color = C.accent }) => {
  const f = useCurrentFrame();
      ...
      {points.map((pt, i) => {
        ...
        return (
          <g key={i} opacity={p}>
            ...
            <text x={px} y={y - 36} textAnchor="middle" fill={C.white} fontSize={36} fontFamily={FONT.title} letterSpacing={1}>{pt.label}</text>
            {pt.sub && <text x={px} y={y + 54} textAnchor="middle" fill={C.gray} fontSize={26} fontFamily={FONT.body}>{pt.sub}</text>}
          </g>
        );
      })}
    </svg>
