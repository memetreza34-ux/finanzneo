...
try {
  ...
  if (!fs.existsSync(path.join(firstProject, '03-szenen', 'BILDER-HIER-EINFUEGEN'))) throw new Error('Bilder-Einfügeordner fehlt.');
  if (!fs.existsSync(path.join(firstProject, '03-szenen', 'EINZELNE-SZENEN'))) throw new Error('Einzelne-Szenen-Ordner fehlt.');
  if (!fs.existsSync(path.join(firstProject, '03-szenen', 'alle-bildprompts.txt'))) throw new Error('Gemeinsame Bildprompt-Datei fehlt.');
  ...
  console.log('✓ Transaktionaler Finance-Scaffold-Test bestanden: neue Struktur, Idempotenz, belegter Tag und Rollback.');
} finally {
