...
export const YouTubeVisual10Animation: React.FC = () => {
  ...
  return <MotionStage>
    <div style={{position:'absolute',left:220,top:220,width:1050,height:600,perspective:1100}}>{blocks.map((p,i)=><div key={i} style={{position:'absolute',left:120+i*220,top:120-i*45,width:520,height:300,borderRadius:36,backgroundColor:i===0?COLORS.green:'#1D6F49',border:'3px solid rgba(255,255,255,0.18)',transform:`translateZ(${i*45}px) scale(${0.84+p*0.16})`,opacity:p,display:'flex',alignItems:'center',justifyContent:'center',fontSize:48,fontWeight:900}}>{i+1}. Monat</div>)}</div>  {/* zone-ok: relativ zum Container in der Zone */}
