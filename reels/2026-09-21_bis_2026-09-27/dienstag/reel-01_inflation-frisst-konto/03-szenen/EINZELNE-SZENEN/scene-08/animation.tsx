...
import {ANIMATION_COLORS, FONT, ease, kursPfad, kursreiheFuerAnimation, settle} from '../../../../../../../src/design-system';
 ...
 * DATA_NARRATIVE
 * CLAIM: Zehn Jahre Weltmarkt steigen auf das 2,93-fache und fallen zwischendurch 24 Prozent unter das vorherige Hoch.
...
export const Scene08Animation: React.FC<{durationFrames?: number}> = ({durationFrames = 255}) => {
  const frame = useCurrentFrame();
  const zeichnen = ease(frame, 18, 150);
  const marke = settle(frame, 154, 176);
  const kennzahlen = settle(frame, 180, 202);
  ...
  return (
    <PremiumPhysicalStage>
      ...
      <PhysicalCoinStack x={150} y={720} coins={muenzen} progress={ease(frame, 8, 34)} />
      ...
      <svg width={1080} height={1920} style={{position: 'absolute', left: 0, top: 0}}>
        <circle cx={ende.x} cy={ende.y} r={16} fill={ANIMATION_COLORS.positive} opacity={ease(frame, 142, 156)} />
      </svg>
      ...
      <div
        style={{
          ...
          opacity: ease(frame, 190, durationFrames - RESULT_HOLD_FRAMES) * 0.85,
        }}
      >
        Quelle: Yahoo Finance · URTH · 10y, Stand 21.9.2026. Historische Entwicklung, keine Aussage ueber die Zukunft.
      </div>
    </PremiumPhysicalStage>
  );
};
