...
## Branch
Arbeite ausschließlich auf dem **aktuell ausgecheckten Arbeitsbranch**. Nicht selbst auf einen älteren Reel-/Systembranch wechseln. `main` nie direkt verändern und nicht mergen.
...
4. Bei echtem Phase-1-/Phase-2-Blocker stoppen und alle Blocker gesammelt nennen.
...
- `REEL_STYLE` in `src/brand/tokens.ts` ist für Layout, Header, Captions und Übergänge verbindlich; abweichende alte Werte im Reel-Index sind nur historische Metadaten und dürfen den zentralen Stil nicht überschreiben
- Header ca. Y=118; zentrale Visualzone ca. Y=390–1560
- Captions V4: satzbasiert, Active hellgrün, Rest weiß, ca. bottom=285, crisp Backplate, kein Glow-Blur, kein Stroke
- Animationen einheitliche Bühne + Start→Mechanismus→Ergebnis
- Übergänge nach `REEL_STYLE`: 3 Frames Continuity, kein Fade-to-black
...
## Export
Nach erfolgreicher QA `npm run reel:export -- <Reel-Pfad> <exakter-gerenderter-MP4-Pfad>` ausführen. Niemals eine fremde oder nur zufällig zuletzt gerenderte MP4 übernehmen.
