...
const styleBlock = `${WORLD_ID_MARKER}\n${SERIES_LOCK_MARKER}\n${GENERATED_IMAGE_ASPECT_MARKER}\nIMAGE_WORLD: ${YOUTUBE_IMAGE_WORLD_ID}\n\nSimple editorial finance illustration. Clean 2D/2.5D visual style. One clear subject. Very few objects. High visual clarity. Flat or lightly dimensional shapes. Clean neutral background. Strong contrast. Professional finance education look. No cinematic scene. No complex 3D environment. No mandatory deep-black world. No tiny details. No decorative finance clutter. No generic coin/piggy-bank composition. No fake app or website. No infographic layout inside the image. No headline, subtitle, explanatory sentence or important number inside the generated image; Remotion owns explanatory text and numbers. Horizontal 16:9.\n`;

const flowStep = (fileName) => `${FLOW_AGENT_PROTOCOL_MARKER}\nCURRENT SINGLE STEP — DO NOT JUMP AHEAD\n\nFINAL FILE NAME:\n${fileName}\n\nGenerate exactly ONE image. Wait until it is fully complete. Rename it immediately to the exact final file name above. Verify the main situation, minimal object count, clean composition, horizontal 16:9 format and exact file name. If any check fails, regenerate the same image number and replace the failed file. Continue only after this image passes. Never render the file name inside the image.\n`;

const promptFor = (index) => {
  const name = visualFileName(index);
  return `${flowStep(name)}\nFLOW_NECESSITY_REASON: [WHY TEXT / NUMBER / ICON / CHART / DIAGRAM / SCREENSHOT / REAL ASSET CANNOT EXPLAIN THIS EQUALLY WELL]\nVOICEOVER_VISUAL_MATCH: [HOW THIS EVERYDAY SCENE DIRECTLY EXPLAINS THE SCRIPT BEAT]\nMAIN_SUBJECT: [ONE CLEAR SUBJECT OR SITUATION]\nSUPPORTING_OBJECTS: [MAXIMUM 2-3 NECESSARY SUPPORTING OBJECTS]\nREMOTION_OVERLAY_TEXT: [IMPORTANT TEXT/NUMBERS TO ADD LATER IN REMOTION, NOT INSIDE THE IMAGE]\n\nIMAGE PROMPT:\nShow [THE EXACT SIMPLE EVERYDAY FINANCE SITUATION]. Keep only the subject and necessary supporting objects.\n\n${styleBlock}`;
};

const thumbnailPrompt = `${flowStep(thumbnailFileName)}\nTHUMBNAIL PROMPT:\nCreate a clean high-impact 16:9 finance YouTube thumbnail background/illustration for [CORE PROMISE OR TENSION]. One focal idea, very few objects, strong contrast, generous clear space for typography that will be added later. Do not generate headline text inside the image.\n\n${styleBlock}`;
...
const visuals = types.map((type, index) => {
  ...
  if (requiresYouTubeImage({type})) {
    ...
    Object.assign(base, {
      ...
      expectedVisual: '[SIMPLE EVERYDAY VISUAL DESCRIPTION]',
      objectLabels: [],
    });
  }
...
write('04-visuals/visual-index.json', `${JSON.stringify({
  ...
  imageWorld:{id:YOUTUBE_IMAGE_WORLD_ID,brandWorldId:WORLD_ID,seriesLockId:SERIES_LOCK_ID,generatedImageAspectRatio:GENERATED_IMAGE_ASPECT_RATIO,horizontalGeneratedImagesRequired:true,referencePromptFile:'04-visuals/bildwelt.txt',styleReferenceStrategy:'simple-editorial-flow-gated',sameWorldAcrossSeriesRequired:true,simpleEditorial:true,cinematic3DDefault:false,remotionOwnsTextAndNumbers:true},
