...
if (existsSync(sceneRoot) && existsSync(indexPath)) {
  ...
  const subtitleMode = index.subtitleMode ?? timing?.subtitleMode;
  const activeWordColor = index.activeWordColor ?? timing?.activeWordColor;
  assert(subtitleMode === 'sentence-with-audio-synced-active-word', 'Audio-synchrone Wortverfolgung fehlt.');
  assert(activeWordColor === 'finance-green', 'Aktive Wortfarbe muss finance-green sein.');
  if (index.imageFit !== undefined) assert(index.imageFit === 'contain', 'imageFit muss contain sein.');
  if (index.maxIntentionalImageScale !== undefined) {
    assert(Number(index.maxIntentionalImageScale) <= 1.05, 'maxIntentionalImageScale darf 1.05 nicht überschreiten.');
  }
  ...
  directories.forEach((id, position) => {
    ...
    if (hasImagePrompt) {
      ...
      assert(typeof indexed?.expectedVisual === 'string' && indexed.expectedVisual.trim().length > 0, `${id}: expectedVisual fehlt.`);
...
console.log('  Ein Satz · aktives Wort grün · Wortzeiten aus Audio · Vordergrundbild contain · max. Scale 1.05');
