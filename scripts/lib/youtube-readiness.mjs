...
const PLACEHOLDER_PATTERN = /\[(?:[^\]]*(?:EINFÜGEN|VOLLSTÄNDIG|KURZER|OPTIONAL|THEMA|NAME|LABEL|METAPHOR|DESCRIBE|PLACE EACH|TITLE|SOURCE|HOOK|VISUAL|CHAPTER|SCRIPT BEAT|CORE|PROMISE|TENSION)[^\]]*)\]/i;
...
export const analyzeYouTubeReadiness = (rootDirectory) => {
  ...
  for (const visual of visuals) {
    ...
    if (typeof visual?.planFile !== 'string' || !visual.planFile.trim()) {
      phase1Blockers.push(`${VISUAL_INDEX}: ${id}.planFile fehlt.`);
    } else {
    ...
    if (visual?.type === 'image') {
      ...
      if (typeof visual.expectedVisual !== 'string' || !visual.expectedVisual.trim() || hasPlaceholder(visual.expectedVisual)) {
    ...
    } else if (visual?.type !== 'animation') {
      phase1Blockers.push(`${VISUAL_INDEX}: ${id}.type muss image oder animation sein.`);
    }
  }
  ...
  if (audioFiles.length === 1 && statSync(resolve(root, '03-audio', audioFiles[0])).size === 0) {
    phase2Blockers.push(`Finales Voiceover ist leer: 03-audio/${audioFiles[0]}`);
  }
  ...
  if (timing) {
    ...
    if (audioFiles.length === 1) {
      ...
      if (!sourceName || sourceName !== audioFiles[0]) {
        phase2Blockers.push(`${WORD_TIMINGS} muss nachweislich aus 03-audio/${audioFiles[0]} erzeugt sein.`);
      }
    }
  }

  if (actualImages.length === expectedImages.length && expectedImages.length > 0) {
    warnings.push('Alle erwarteten 16:9-Nutzerbilder sind vorhanden.');
  }
