...
const imagePrompt = (id, index) => `${flowInstruction(sceneFileName(index))}\n${STYLE_BLOCK}\nHEADLINE + SUBLINE – EXAKT SO INS BILD EINBRENNEN:\nHeadline: [KURZE FETTE HEADLINE, 3-7 WÖRTER]\nSubline: [EIN KURZER SATZ DARUNTER]\n\nBESCHRIFTUNGEN – OPTIONAL:\n- [KURZES DEUTSCHES OBJEKT-LABEL]\n- [OPTIONALES ZWEITES KURZES LABEL]\n\nBILDPROMPT:\nA stylized 3D adult person with a clearly visible stylized face, front-facing or in a natural three-quarter view, standing beside [ONE LARGE DOMINANT VISUAL METAPHOR FOR ${id}]. [DESCRIBE ONE CLEAR CAUSE-AND-EFFECT ACTION USING ONLY A FEW LARGE OBJECTS]. Bake the headline and subline above into the upper third of the image. Include German object labels: [PLACE EACH SHORT LABEL DIRECTLY BESIDE THE RELEVANT OBJECT].\n`;

const coverPrompt = `${flowInstruction('Bild 00 - [KURZER COVER-NAME].png')}\n${STYLE_BLOCK}\nCOVER-REGEL:\nGleiche Textregel wie jedes Szenenbild: EIN starkes Hauptmotiv plus eingebrannte Headline und Subline im oberen Drittel.\n\nHEADLINE + SUBLINE – EXAKT SO INS BILD EINBRENNEN:\nHeadline: [KURZE FETTE HEADLINE ZUM THEMA, 3-7 WÖRTER]\nSubline: [EIN KURZER SATZ DARUNTER]\n\nBESCHRIFTUNGEN – OPTIONAL:\n- [OPTIONALE KURZE STRUKTUR-LABELS]\n\nBILDPROMPT:\nA stylized 3D adult person with a clearly visible stylized face, front-facing or in a natural three-quarter view, standing beside [ONE LARGE DOMINANT COVER METAPHOR]. [SHOW THE CORE IDEA OF THE REEL IN ONE CLEAR VISUAL]. Include only the specified short German object labels directly beside their objects.\n`;
