...
if(!targetArg){console.error('Nutzung: npm run reel:create -- --target reels/<Woche>/<Tag>/<Reel> --title "Titel" [--types image,image,animation]');process.exit(1);}
if(types.length<5||types.length>12||types.some((t)=>!['image','animation'].includes(t))){console.error('Szenentypen: 5–12 Einträge, nur image oder animation.');process.exit(1);}

const root=resolve(targetArg);
const rel=relative(resolve('reels'),root);
if(rel.startsWith('..')||rel===''||rel.split(sep).includes('..')){console.error('Ziel muss ein neuer Ordner unter reels/ sein.');process.exit(1);}
if(existsSync(root)){console.error(`Ziel existiert bereits: ${root}`);process.exit(1);}

const write=(path,content)=>{const abs=resolve(root,path);mkdirSync(resolve(abs,'..'),{recursive:true});writeFileSync(abs,content,'utf8');};
...
const coverPrompt=`${flowInstruction('Bild 00 - [KURZER COVER-NAME].png')}\n${STYLE_BLOCK}\nCOVER-REGEL:\nKeine klassische Überschrift. Thema über EIN starkes Hauptmotiv + wenige kurze Objekt-Beschriftungen erklären.\n\nBILDPROMPT:\nA stylized 3D adult person with a clearly visible stylized face, front-facing or in a natural three-quarter view, standing beside [ONE LARGE DOMINANT COVER METAPHOR]. [SHOW THE CORE IDEA IN ONE CLEAR VISUAL]. Include only explicitly specified short German object labels. ${STYLE_BLOCK}\n`;
...
write('03-szenen/00-ALLE-BILDER-HIER-REIN/README.md','# ALLE FERTIGEN NUTZERBILDER HIER REIN\n\nNur die exakt erwarteten finalen Bilder ablegen. Keine Zusatzbilder, Platzhalter oder Altversionen. Animationsszenen haben kein Bild und ihre Nummer bleibt reserviert.\n');
...
write('04-caption/word-timings.json',`${JSON.stringify({version:2,fps:30,subtitleMode:'sentence-with-audio-synced-active-word',activeWordColor:'finance-green',timingStatus:'missing-final-audio-alignment',timingMethod:'real-word-boundaries-required',rules:{preferredSentencesVisible:1,maxSentencesVisible:2,maxLines:2,sentenceSwitch:'next-sentence-first-word-start',holdPreviousSentenceDuringShortPause:true,equalWordSpacingForbidden:true},sentences:[]},null,2)}\n`);
...
write('05-projektdateien/technische-hinweise.md','# TECHNISCHE HINWEISE\n\n- 1080 × 1920, 30 fps\n- Bildszenen: adaptive-safe-fill, nicht contain\n- Bildfläche so groß wie möglich zwischen Headline und Caption\n- bevorzugt Visualbereich ungefähr Y 210–1515\n- zuerst nur leeren nahtlosen Hintergrund croppen; Gesicht, Objektlabels, Hero-Objekt und Geld/Wert schützen\n- kein sichtbares Bild-im-Bild-Panel und keine unscharfe Bildkopie\n- Untertitel bevorzugt 1 Satz gleichzeitig, maximal 2 sehr kurze Sätze und hart maximal 2 Zeilen\n- Untertitel ungefähr 280 px über dem unteren Rand, rechts extra UI-Sicherheitsabstand\n- Wortmarkierung ausschließlich nach echten start/end-Zeitstempeln des finalen Voiceovers\n- gleichmäßig geschätzte Wortzeiten verboten\n- kurze Sprachpausen: bisherigen Satz halten, Satzwechsel erst beim ersten gesprochenen Wort des nächsten Satzes\n- Publishing nur Instagram Reels, TikTok, Facebook Reels, Snapchat\n- keine YouTube Shorts; YouTube nur Longform unter youtube/\n- Audioziel ungefähr -16 LUFS, True Peak höchstens -1 dBTP\n');
write('05-projektdateien/timeline.json',`${JSON.stringify({version:2,title,fps:30,timingSource:'04-caption/word-timings.json',cutRule:'voice-sentence-start',wordTimingSource:'final-audio-only',scenes:types.map((type,index)=>({id:`scene-${num(index)}`,type,startFrame:0,durationFrames:0,cutReason:'voice-sentence-start'}))},null,2)}\n`);

const scenes=types.map((type,index)=>{
  const number=num(index);const id=`scene-${number}`;const dir=`03-szenen/EINZELNE-SZENEN/${id}`;
  ...
  if(type==='image'){
    ...
    return {...common,planFile:`EINZELNE-SZENEN/${id}/bildprompt.txt`,googleFlowFileName:sceneFileName(index),objectLabels:['[EINFÜGEN]'],expectedVisual:'[EINFÜGEN]',imagePresentation:{mode:'adaptive-safe-fill',focalX:0.5,focalY:0.52}};
  }
...
const allSections=types.map((type,index)=>{const number=num(index);if(type==='animation')return `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\nSZENE ${number} – REMOTION-ANIMATION\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\nKEIN BILD ${number} ERZEUGEN. Nummer ${number} bleibt reserviert.\n`;return `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\nSZENE ${number} – BILDSZENE\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n${imagePrompt(`scene-${number}`,index)}`;}).join('\n');
...
write('03-szenen/scene-index.json',`${JSON.stringify({version:13,title,sceneCount:scenes.length,imageSceneCount:imageSceneIds.length,animationSceneCount:animationSceneIds.length,fps:30,userCreatesImages:true,antigravityGeneratesImages:false,subtitleDisplay:{preferredSentences:1,maxSentences:2,maxLines:2,balancedLines:true,holdDuringPauses:true,noDeadGaps:true,timingSource:'real-audio-word-timestamps',activeWordTiming:'exact-word-start-end',sentenceSwitch:'next-sentence-first-word-start',equalWordSpacingForbidden:true},layout:{headlineTop:70,visualTop:210,visualBottom:1515,subtitleBottom:280,subtitleLeft:60,subtitleRight:180,platformUiSafeBottom:260},googleFlow:{generationMode:'one-image-at-a-time',fileNameRule:'Bild XX - Kurzer Szenenname.png',numberSource:'real-scene-number',animationNumbersStayReserved:true,finalCollectionDirectory:'03-szenen/00-ALLE-BILDER-HIER-REIN/',distributeToSceneFolders:false},userMediaBoundary:{imagesDirectory:'03-szenen/00-ALLE-BILDER-HIER-REIN/',audioDirectory:'02-audio/',outsideMediaForbidden:true,substitutesForbidden:true,missingRequiredMediaIsBlocker:true},imageWorld:{id:WORLD_ID,referencePromptFile:'03-szenen/bildwelt.txt',style:'premium-fintech-editorial-3d-metaphor',stylizedPersonAllowed:true,visibleFaceRequiredWhenPersonPresent:true,objectLabelsOnly:true,seamlessSingleBackgroundRequired:true,percentageZonesForbidden:true,floorWallBoundaryForbidden:true,horizonLineForbidden:true,backgroundBandsForbidden:true,headlinesInGeneratedImagesForbidden:true,subtitlesInGeneratedImagesForbidden:true,sentencesInGeneratedImagesForbidden:true,tinyDioramaForbidden:true,neonTunnelForbidden:true},platformPublishing:{directory:'04-caption',masterCaption:'04-caption/caption.txt',instagramReels:'04-caption/instagram-reels.txt',tiktok:'04-caption/tiktok.txt',facebookReels:'04-caption/facebook-reels.txt',snapchat:'04-caption/snapchat.txt'},timelineRules:{timingSource:'04-caption/word-timings.json',cutsFollowSentenceStarts:true,equalLengthScenesForbiddenByDefault:true,wordTimingsMustComeFromFinalAudio:true,evenlyDistributedWordTimingsForbidden:true},audio:{targetIntegratedLufs:-16,targetTruePeakDbtp:-1},imagePresentationContract:{mode:'adaptive-safe-fill',maximizeVisualArea:true,preferCropEmptyBackground:true,preserveFace:true,preserveObjectLabels:true,preserveHeroObject:true,preserveMoneyAndValue:true,blurredImageBackgroundForbidden:true,visibleInsetPanelForbidden:true,containAsDefaultForbidden:true},scenes},null,2)}\n`);
...
console.log('  Darstellung: adaptive-safe-fill · maximale Bildfläche · kein contain-Panel');
console.log('  Captions: echte Audio-Wortzeiten · bevorzugt 1 Satz · max. 2 Zeilen');
