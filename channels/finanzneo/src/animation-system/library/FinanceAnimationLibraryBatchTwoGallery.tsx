...
export const FINANCE_ANIMATION_LIBRARY_BATCH_TWO_ITEMS = getFinanceAnimationLibraryItemsByBatch(2);
export const FINANCE_ANIMATION_LIBRARY_BATCH_TWO_ITEM_DURATION = 180;
export const FINANCE_ANIMATION_LIBRARY_BATCH_TWO_DURATION =
  FINANCE_ANIMATION_LIBRARY_BATCH_TWO_ITEMS.length * FINANCE_ANIMATION_LIBRARY_BATCH_TWO_ITEM_DURATION;

export const FinanceAnimationLibraryBatchTwoGallery: React.FC = () => (
  <AbsoluteFill style={{background: '#06110A'}}>
    {FINANCE_ANIMATION_LIBRARY_BATCH_TWO_ITEMS.map((item, index) => (
      <Sequence
        key={item.id}
        from={index * FINANCE_ANIMATION_LIBRARY_BATCH_TWO_ITEM_DURATION}
...
export const FinanceAnimationLibraryBatchTwoOverview: React.FC = () => (
  <AbsoluteFill
    style={{
  ...
  >
    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'end', marginBottom: 28}}>
      <div>
        ...
        <div style={{fontSize: 52, fontWeight: 950, marginTop: 6}}>Animationsbibliothek · Batch 2</div>
      </div>
      <div style={{fontSize: 25, color: '#AFC0B4'}}>{FINANCE_ANIMATION_LIBRARY_BATCH_TWO_ITEMS.length} neue Animationen</div>
    </div>
        ...
        return (
          ...
          >
            ...
            >
              ...
              >
                <Sequence durationInFrames={FINANCE_ANIMATION_LIBRARY_BATCH_TWO_ITEM_DURATION} width={SOURCE_WIDTH} height={SOURCE_HEIGHT}>
                  {item.renderDemo()}
                </Sequence>
              </div>
            </div>
